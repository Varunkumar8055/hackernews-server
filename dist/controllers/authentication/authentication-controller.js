...
const index_1 = require("../../integrations/prisma/index");
...
const signUpWithUsernameAndPassword = async (parameters) => {
    try {
        const existingUser = await index_1.prismaClient.user.findUnique({
            where: {
        ...
        const result = await index_1.prismaClient.$transaction(async (tx) => {
...
const logInWithUsernameAndPassword = async (parameters) => {
    ...
    await index_1.prismaClient.session.create({
        data: {
            id: (0, crypto_1.randomUUID)(),
            token: sessionToken,
            expiresAt,
